...
PostProcessor::PostProcessor() : DataPostprocessorVector<2>("current_density", update_values | update_gradients) {}
...
compute_derived_quantities_vector (	const std::vector< Vector< double > > &  	uh,
	...
	for (unsigned int i=0; i<computed_quantities.size(); i++) {
		...
		for (unsigned int d=0; d<2; ++d) {
			...
			computed_quantities[i](d) = 5/std::max(t,300.0)*e_field;
		}
	}
}
...
template <int dim>
class EmissionCurrent : public Function<dim> {
public:
	EmissionCurrent() : Function<dim>(2) {} // The number of components needs to be passed to Function, probably...
	virtual double value (const Point<dim> &p, const unsigned int component = 0) const;
};
template <int dim>
double EmissionCurrent<dim>::value (const Point<dim> &p, const unsigned int /*component */) const {
	// electrons fly out -> current is inwards
	return 5*(1+(p[1]+4.0)*2)*1e-4;
}
...
template <int dim>
class ElectricConductivity: public Function<dim> {
public:
	ElectricConductivity() : Function<dim>(2) {}
	virtual double value (const Point<dim> &p, const unsigned int component = 0) const;
};
template <int dim>
double ElectricConductivity<dim>::value (const Point<dim> &p, const unsigned int /*component */) const {
	return p[0]*p[0]+10;
}

template <int dim>
class ThermalConductivity: public Function<dim> {
public:
	ThermalConductivity() : Function<dim>(2) {}
	virtual double value (const Point<dim> &p, const unsigned int component = 0) const;
};
template <int dim>
double ThermalConductivity<dim>::value (const Point<dim> &p, const unsigned int /*component */) const {
	return p[0]+10.0;
}

double CurrentsAndHeating::el_cond(double t) {
	// NB: postprocessor uses a copy of this, to calculate current density
	// If change, it should be also changed there...
	return 5/std::max(t,300.0);
}
double CurrentsAndHeating::th_cond(double t) {
	return 0.5/std::max(t,300.0);
}

double CurrentsAndHeating::em_current(const Point<2> &p, double t) {
	double e_field = laplace_problem.probe_field(p);
	//std::cout << e_field << std::endl;
	double j_e = e_field*e_field*std::exp(-1/e_field)/(80.0*3.0*3.0); // field emission
	double j_t = t*t/(1000.0*1500.0*1500.0); //thermionic emission
	return j_e+j_t;
}
...
void CurrentsAndHeating::assemble_system() {
	...
	const EmissionCurrent<2> emission_current;
	const ElectricConductivity<2> electric_conductivity;
	const ThermalConductivity<2> thermal_conductivity;

	std::vector<double> el_conductivity_values (n_q_points);
	std::vector<double> th_conductivity_values (n_q_points);
	...
	for (; cell != endc; ++cell) {
		...
		thermal_conductivity.value_list(fe_values.get_quadrature_points(), th_conductivity_values);
		...
		for (unsigned int q = 0; q < n_q_points; ++q) {
			for (unsigned int i = 0; i < dofs_per_cell; ++i) {
				...
				for (unsigned int j = 0; j < dofs_per_cell; ++j) {
					...
					cell_matrix(i, j) += (	- (grad_phi_i_p * el_cond(prev_sol_temperature_values[q]) * grad_phi_j_p)
											+ (phi_i_t * el_cond(prev_sol_temperature_values[q])
												* prev_sol_potential_gradients[q] * grad_phi_j_p)
											- (grad_phi_i_t * th_cond(prev_sol_temperature_values[q]) * grad_phi_j_t)
										 ) * fe_values.JxW(q);
