...
Mesher::Mesher(string mesher) {
	if(mesher != "tetgen")
		cout << "Unknown mesher: " + mesher << endl;
}
...
void Mesher::smooth_mesh(int nrOfIterations) {
	REAL* node = tetgenOut.pointlist;		// pointer to nodes
...
void Mesher::clean_elements(tetgenio* tetIO, double rmax) {
	...
	for (i = 0; i < N; ++i) {
		...
		for (k = 0; k < 3; ++k) {
			...
			dx = node[l1] - node[l2]; r12 += dx * dx; // length^2 of tetrahedron 1st edge
			dx = node[l1] - node[l3]; r13 += dx * dx; // ..2nd edge
			dx = node[l1] - node[l4]; r14 += dx * dx; // ..3rd edge
			dx = node[l2] - node[l3]; r23 += dx * dx; // ..4th edge
			dx = node[l2] - node[l4]; r24 += dx * dx; // ..5th edge
			dx = node[l3] - node[l4]; r34 += dx * dx; // ..6th edge
		}
		// Keep only the elements with appropriate quality
		isQuality[i] = (r12<rmax) & (r13<rmax) & (r14<rmax) & (r23<rmax) & (r24<rmax) & (r34<rmax);
	}
...
void Mesher::clean_faces(tetgenio* tetIO, double rmax) {
	...
	for (i = 0; i < N; ++i) {
		...
		for (k = 0; k < 3; ++k) {
			...
			dx = node[l1] - node[l2]; r12 += dx * dx; // length^2 of face's 1st edge
			dx = node[l1] - node[l3]; r13 += dx * dx; // ..2nd edge
			dx = node[l2] - node[l3]; r23 += dx * dx; // ..3rd edge
		}
...
void Mesher::clean_edges(tetgenio* tetIO, double rmax) {
	...
	for (i = 0; i < N; ++i) {
		...
		for (k = 0; k < 3; ++k) {
			...
			dx = node[l1] - node[l2]; r12 += dx * dx; // length^2 of edge
		}
...
void Mesher::update_list(int* list, int* listSize, vector<bool> isQuality, int M) {
	int i, j, k;
	int N = isQuality.size();
	// new list size = number of objects with appropriate quality
	*listSize = accumulate(isQuality.begin(), isQuality.end(), 0);
	j = 0;
