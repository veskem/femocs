...
SurfaceExtractor::SurfaceExtractor(const string& adapter, const double cutoff, const int nnn, const int nt) {
...
shared_ptr<Surface> SurfaceExtractor::extract_surface(const AtomReader::Data data) {
...
bool SurfaceExtractor::isBoundary(const int i, const AtomReader::Data dat) {
	return (dat.xmax-abs(dat.x[i])<surfEps) | (dat.ymax-abs(dat.y[i])<surfEps) | (dat.z[i]-dat.zmin<surfEps);
}

// Extract surface by coordination analysis
shared_ptr<Surface> SurfaceExtractor::coordination_extract(const AtomReader::Data dat) {
	...
	for (i = 0; i < nrOfAtoms; ++i)
		if ( isSurface[i] )
			s->add(dat.x[i], dat.y[i], dat.z[i], coords[i], dat.type[i]);
