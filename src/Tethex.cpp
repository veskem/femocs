 ...
 *
 * Modified: February 2016 Mihkel Veske, mihkel.veske@helsinki.fi
 *
...
double Point::get_coord(int number) const {
    expect(number >= 0 && number < n_coord,
            "The number of coordinate is incorrect: " + d2s(number) + ". It should be in the range: [0, " + d2s(n_coord) + ")");
...
MeshElement::MeshElement(int n_ver, int n_edg, int n_fac, int el_type) :
        n_vertices(n_ver), vertices(), n_edges(n_edg), edges(), n_faces(n_fac), faces(), material_id(
                0), gmsh_el_type(el_type) {
...
MeshElement::MeshElement(const MeshElement &elem) :
        n_vertices(elem.n_vertices), vertices(elem.vertices), n_edges(elem.n_edges), edges(
                elem.edges), n_faces(elem.n_faces), faces(elem.faces), material_id(
                elem.material_id), gmsh_el_type(elem.gmsh_el_type) {
}
...
int MeshElement::get_vertex(int number) const {
    expect(number >= 0 && number < n_vertices,
            "The local number of vertex is incorrect: " + d2s(number) + ". It has to be in range [0, " + d2s(n_vertices) + ").");
    return vertices[number];
}

int MeshElement::get_edge(int number) const {
    expect(number >= 0 && number < n_edges,
            "The local number of edge is incorrect: " + d2s(number) + ". It has to be in range [0, " + d2s(n_edges) + ").");
    return edges[number];
}

int MeshElement::get_face(int number) const {
    expect(number >= 0 && number < n_faces,
            "The local number of face is incorrect: " + d2s(number) + ". It has to be in range [0, " + d2s(n_faces) + ").");
    return faces[number];
}
...
void MeshElement::set_face(int local_number, int global_number) {
    expect(local_number >= 0 && local_number < get_n_faces(),
            "Local number (" + d2s(local_number) + ") is incorrect. It must be in the range [0, " + d2s(n_faces) + ")");
...
PhysPoint::PhysPoint(const std::vector<int> &ver, int mat_id) :
    ...
    expect(vertices.size() == ver.size(),
            "The size of list of vertices (" + d2s(ver.size()) + ") is not equal to really needed number of vertices (" + d2s(n_vertices) + ")");
    vertices = ver;
    material_id = mat_id;
}
...
int Line::common_vertex(const Line& line) const {
    for (int i = 0; i < n_vertices; ++i)
        if (line.contains(vertices[i])) return vertices[i];
...
Hexahedron::Hexahedron(int v1, int v2, int v3, int v4, int v5, int v6, int v7, int v8, int mat_id) :
        MeshElement(n_vertices, n_edges, n_faces, gmsh_el_type) {
    vertices[0] = v1;
    vertices[1] = v2;
    vertices[2] = v3;
...
//
//-------------------------------------------------------
IncidenceMatrix::IncidenceMatrix(int n_vertices, const std::vector<MeshElement*> &cells) :
        dim(n_vertices), n_non_zero(0), row(nullptr), col(nullptr) {
...
int IncidenceMatrix::find(int row_number, int col_number) const {
    // because we seek in lower triangle, row must be bigger than col
    expect(row_number > col_number,
            "We seek values in lower triangle, so row should be bigger than column. But in this case row_number = " + d2s(row_number) + ", col_number = " + d2s(col_number) + "!");

    for (int i = row[row_number]; i < row[row_number + 1]; ++i) {
        if (col[i] == col_number) return i;
    }
...
void Mesh::read_ascii(ifstream &in, int flags) {
    ...
    bool read_edges = flags & (1 << 0);
    bool read_all_faces = flags & (1 << 1);
...
void Mesh::read_bin(ifstream &in, const int flags) {
    ...
    while (in >> str) {
        ...
        if (str == "$Nodes") {
            ...
            femocs::write_verbose_msg("Reading " + d2s(n_vertices) + " vertices");
            ...
            for (int ver = 0; ver < n_vertices; ++ver) {
                ...
                expect(number == ver, "Mismatch between number and ver: " + d2s(number) + " vs " + d2s(ver));
                vertices_map[number] = ver; // add the number of vertex to the map
            }
        ...
        // read the mesh elements
            ...
            // read the total number of mesh elements
            int sum_n_elements;
            in >> sum_n_elements;
            getline(in, str); // read line feed

            int header[3];  // the header of the element
            int phys_domain;// physical domain of element
            ...
            while (n_read_elements < sum_n_elements) {
                ...
                require(n_tags >= 1,
                        "Physical domain of elements of type " + d2s(el_type) +
                        " missing, as # element tags is too small: " + d2s(n_tags));

                femocs::write_verbose_msg("Reading " + d2s(n_elements) + " elements of type " + d2s(el_type));
                n_read_elements += n_elements;

                int n_elem_nodes = 0;
                if (el_type == femocs::Gmsh::line)
                    n_elem_nodes = femocs::n_nodes_per_edge;
                else if (el_type == femocs::Gmsh::triangle)
                    n_elem_nodes = femocs::n_nodes_per_tri;
                else if (el_type == femocs::Gmsh::tetrahedron)
                    n_elem_nodes = femocs::n_nodes_per_tet;

                int n_data = 1 + n_tags + n_elem_nodes; // how much data we need to read
                int data[n_data];
                vector<int> nodes(n_elem_nodes);

                // read the elements of given type
                for (int el = 0; el < n_elements; ++el) {
                    in.read(reinterpret_cast<char*>(data), sizeof(data)); // read the data
                    serial_nr = data[0];
                    phys_domain = data[1];
                    for (int i = 0; i < n_elem_nodes; ++i)
                    ...
                    switch (el_type) {
                    case femocs::Gmsh::line:  // 2-node line
                        lines.push_back(new Line(nodes, phys_domain));
                        break;
                    case femocs::Gmsh::triangle: // 3-node triangle
                        triangles.push_back(new Triangle(nodes, phys_domain));
                        break;
                    case femocs::Gmsh::tetrahedron: // 4-node tetrahedron
                        tetrahedra.push_back(new Tetrahedron(nodes, phys_domain));
                          break;
...
void Mesh::read_femocs(femocs::TetgenMesh* mesh) {
    ...
    if (n_faces == mesh->tris.get_n_markers())
        for (int f = 0; f < n_faces; ++f)
            triangles.push_back( new Triangle(mesh->tris[f].to_vector(), mesh->tris.get_marker(f)) );
    else
        for (int f = 0; f < n_faces; ++f)
    ...
    if (n_elements == mesh->tets.get_n_markers())
        for (int el = 0; el < n_elements; ++el)
            tetrahedra.push_back( new Tetrahedron(mesh->tets[el].to_vector(), mesh->tets.get_marker(el)) );
    else
        for (int el = 0; el < n_elements; ++el)
            tetrahedra.push_back( new Tetrahedron(mesh->tets[el].to_vector()) );
...
void Mesh::export_femocs(femocs::TetgenMesh* mesh) {
    ...
    for (int vert = 0; vert < n_verts; ++vert) {
        ...
        mesh->nodes.append(femocs::Point3(p.get_coord(0), p.get_coord(1), p.get_coord(2)));
...
void Mesh::convert() {
    if (!tetrahedra.empty())
        convert_3D();
    else if (!triangles.empty()) convert_2D();

    if (!hexahedra.empty()) convert_hexahedra();
    ...
    for (size_t tet = 0; tet < tetrahedra.size(); ++tet) {
        for (int ver = 0; ver < Tetrahedron::n_vertices; ++ver) {
            ...
            for (int edge = 0; edge < Tetrahedron::n_edges; ++edge) {
                ...
                if (edges[cur_edge]->contains(cur_vertex)) seek_edges.push_back(cur_edge);
            }
...
void Mesh::convert_triangles(const IncidenceMatrix &incidence_matrix, int n_old_vertices,
        bool numerate_edges, const VectorMap &edge_vertex_incidence) {
...
//        std::swap(quadrangle_vertices[1], quadrangle_vertices[3]);
            ...
            quadrangles.push_back(
                    new Quadrangle(quadrangle_vertices, triangles[tri]->get_material_id()));
...
void Mesh::convert_quadrangles() {
    for (size_t elem = 0; elem < quadrangles.size(); ++elem) {
        ...
        if (quad_vertices[1] != ver) ++n_converted_quadrangles;
...
void Mesh::convert_hexahedra() {
    for (size_t elem = 0; elem < hexahedra.size(); ++elem) {
        ...
        // we compare only one vertex number
        if (hexahedron_vertices[0] != ver) ++n_converted_hexahedra;
...
void Mesh::redefine_lines(const IncidenceMatrix &incidence_matrix, int n_old_vertices) {
    ...
    for (int line = 0; line < n_old_lines; ++line) {
        ...
        const int edge = incidence_matrix.find(std::max(ver1, ver2), std::min(ver1, ver2));
        ...
        lines.push_back(new Line(n_old_vertices + edge, ver2, lines[line]->get_material_id())); // add new line
    }
...
void Mesh::edge_numeration(std::vector<MeshElement*> &cells,
        const IncidenceMatrix &incidence_matrix, bool initialize_edges) {
    ...
    if (initialize_edges) edges.resize(n_edges);
    ...
    for (size_t cell = 0; cell < cells.size(); ++cell) {
        ...
        for (int i = 0; i < cells[cell]->get_n_vertices(); ++i) {
            ...
            for (int j = 0; j < cells[cell]->get_n_vertices(); ++j) {
                ...
                if (ii > jj) // ii must be bigger than jj
                    ...
                    if (initialize_edges) {
                        delete edges[gne];
                        edges[gne] = new Line(std::min(ii, jj), std::max(ii, jj),
        ...
        expect(lne == cells[cell]->get_n_edges(),
                "lne must be equal to " + d2s(cells[cell]->get_n_edges()) + ", but it is " + d2s(lne));
    }

} // edge numeration

void Mesh::face_numeration(std::vector<MeshElement*> &cells,
        const IncidenceMatrix &incidence_matrix, VectorMap &edge_vertex_incidence) {
            ...
            for (int ver = 0; ver < cells[cell]->get_n_vertices(); ++ver) {
                ...
                if (!edges[cur_edge]->contains(cur_vertex)) // if edge doesn't contain vertex - they are opposite to each other
                    ...
                    if (edge_vertex_incidence[cur_edge].find(cur_vertex)
                        ...
                        edge_vertex_incidence[another_edge][edges[cur_edge]->get_vertex(1)] =
                                n_faces;
                        ...
                        edge_vertex_incidence[another_edge][edges[cur_edge]->get_vertex(0)] =
                                n_faces;
                        ...
                        faces.push_back(
                                new Triangle(face_vertices, cells[cell]->get_material_id()));
                    ...
                    } else // this face already exsists
                    {
                        ...
                        if (find(face_numbers.begin(), face_numbers.end(), fnumber)
                                == face_numbers.end()) face_numbers.push_back(fnumber);
                    }
...
//    const int n_all_elements = points.size() + lines.size() + triangles.size() + tetrahedra.size()
//            + quadrangles.size() + hexahedra.size();
    const int n_all_elements = quadrangles.size() + hexahedra.size();
    out << "$EndNodes\n$Elements\n" << n_all_elements << "\n";
...
int Mesh::find_face_from_two_edges(int edge1, int edge2, const IncidenceMatrix &vertices_incidence,
        const VectorMap &edge_vertex_incidence) const {
    // initialize auxiliary lines
    Line line1(edges[edge1]->get_vertex(0), edges[edge1]->get_vertex(1),
            edges[edge1]->get_material_id());
    Line line2(edges[edge2]->get_vertex(0), edges[edge2]->get_vertex(1),
...
//
//-------------------------------------------------------
void write_elements(std::ostream &out, const std::vector<MeshElement*> &elems, int &serial_number) {
...
void change_vertices_order(int dimension, const std::vector<Point> &all_mesh_vertices,
...
double cell_measure_2D(const std::vector<Point> &vertices, const std::vector<int> &indices) {
    const double x[] = { vertices[indices[0]].get_coord(0), vertices[indices[1]].get_coord(0),
            vertices[indices[2]].get_coord(0), vertices[indices[3]].get_coord(0) };
    const double y[] = { vertices[indices[0]].get_coord(1), vertices[indices[1]].get_coord(1),
            vertices[indices[2]].get_coord(1), vertices[indices[3]].get_coord(1) };
    return (-x[1] * y[0] + x[1] * y[3] + y[0] * x[2] + x[0] * y[1] - x[0] * y[2] - y[1] * x[3]
            - x[2] * y[3] + x[3] * y[2]) / 2.;
}

double cell_measure_3D(const std::vector<Point> &vertices, const std::vector<int> &indices) {
    const double x[8] = { vertices[indices[0]].get_coord(0), vertices[indices[1]].get_coord(0),
            vertices[indices[2]].get_coord(0), vertices[indices[3]].get_coord(0),
            vertices[indices[4]].get_coord(0), vertices[indices[5]].get_coord(0),
            vertices[indices[6]].get_coord(0), vertices[indices[7]].get_coord(0) };
    const double y[8] = { vertices[indices[0]].get_coord(1), vertices[indices[1]].get_coord(1),
            vertices[indices[2]].get_coord(1), vertices[indices[3]].get_coord(1),
            vertices[indices[4]].get_coord(1), vertices[indices[5]].get_coord(1),
            vertices[indices[6]].get_coord(1), vertices[indices[7]].get_coord(1) };
    const double z[8] = { vertices[indices[0]].get_coord(2), vertices[indices[1]].get_coord(2),
            vertices[indices[2]].get_coord(2), vertices[indices[3]].get_coord(2),
            vertices[indices[4]].get_coord(2), vertices[indices[5]].get_coord(2),
            vertices[indices[6]].get_coord(2), vertices[indices[7]].get_coord(2) };
    ...
    const double t34 = z[3] * x[1] * y[2] + t3 * z[1] - t5 * y[7] + y[7] * x[4] * z[6] + t9 * y[6]
            - t11 * z[4] - t5 * y[3] - t14 * z[2] + z[1] * x[4] * y[0] - t18 * z[3] + t20 * z[0]
            - t22 * z[0] - y[0] * x[5] * z[4] - t26 * y[3] + t28 * z[2] - t9 * y[1]
            - y[1] * x[4] * z[0] - t11 * z[5];
    ...
    const double t64 = -x[6] * y[4] * z[2] - t37 * z[2] + t18 * z[6] - x[3] * y[6] * z[2]
            ...
            - t22 * z[1] + t52 * z[3] - t54 * z[2] - t56 * y[4] - t58 * z[0] + y[1] * x[2] * z[0]
            + t9 * y[7] + t37 * z[4];
    ...
    const double t95 = -t66 * z[3] - t68 * z[2] - t70 * z[2] + t26 * y[5] - t73 * y[6] - t14 * z[6]
            + t76 * z[2] - t3 * z[6] + x[6] * y[2] * z[4] - z[3] * x[6] * y[2] + t26 * y[4]
            - t44 * z[3] - x[1] * y[2] * z[0] + x[5] * y[6] * z[4] + t54 * z[5] + t90 * y[2]
            - t92 * z[2] + t46 * y[2];
    const double t102 = x[2] * y[0];
    const double t107 = y[3] * x[7];
    const double t114 = x[0] * y[6];
    const double t125 = y[0] * x[3] * z[2] - z[7] * x[5] * y[6] - x[2] * y[6] * z[4] + t102 * z[6]
            - t52 * z[6] + x[2] * y[4] * z[6] - t107 * z[5] - t54 * z[6] + t58 * z[6]
            - x[7] * y[4] * z[6] + t37 * z[5] - t114 * z[4] + t102 * z[4] - z[1] * x[2] * y[0]
            + t28 * z[6] - y[5] * x[6] * z[4] - z[5] * x[1] * y[4] - t73 * y[7];
    ...
    const double t156 = t90 * y[6] - t129 * y[4] + z[7] * x[2] * y[6] - t133 * z[5]
            + x[5] * y[3] * z[7] - t26 * y[2] - t70 * z[3] + t46 * y[3] + z[5] * x[7] * y[4]
            + z[7] * x[3] * y[6] - t49 * z[4] + t145 * z[7] - x[2] * y[7] * z[6] + t70 * z[5]
            + t66 * z[5] - z[7] * x[4] * y[6] + t18 * z[4] + x[1] * y[4] * z[0];
    ...
    const double t181 = t107 * z[6] + t22 * z[7] + t76 * z[3] + t160 * z[1] - x[4] * y[2] * z[6]
            + t70 * z[4] + t165 * y[5] + x[7] * y[2] * z[6] - t76 * z[5] - t76 * z[4] + t133 * z[3]
            - t58 * z[1] + y[5] * x[0] * z[4] + t114 * z[2] - t3 * z[7] + t20 * z[2] + t178 * y[7]
            + t129 * y[2];
    const double t207 = t92 * z[7] + t22 * z[6] + z[3] * x[0] * y[2] - x[0] * y[3] * z[2]
            - z[3] * x[7] * y[2] - t165 * y[3] - t9 * y[0] + t58 * z[7] + y[3] * x[6] * z[2]
            + t107 * z[2] + t73 * y[0] - x[3] * y[5] * z[7] + t3 * z[0] - t56 * y[6]
            - z[5] * x[0] * y[4] + t73 * y[1] - t160 * z[6] + t160 * z[0];
    const double t228 = -t44 * z[7] + z[5] * x[6] * y[4] - t52 * z[4] - t145 * z[4] + t68 * z[4]
            + t92 * z[5] - t92 * z[0] + t11 * z[3] + t44 * z[0] + t178 * y[5] - t46 * y[5]
            - t178 * y[0] - t145 * z[0] - t20 * z[5] - t37 * z[3] - t160 * z[7] + t145 * z[3]
            + x[4] * y[6] * z[2];
